...
module Toggle = {
  ...
  module Bounded_Join_Semilattice:
    BOUNDED_JOIN_SEMILATTICE with type t = toggle = {
    include Join_Semilattice;
    let bottom = Off;
  };

  module Bounded_Meet_Semilattice:
    BOUNDED_MEET_SEMILATTICE with type t = toggle = {
  ...
  module Bounded_Lattice: BOUNDED_LATTICE with type t = toggle = {
    ...
    include (
              Bounded_Meet_Semilattice:
                BOUNDED_MEET_SEMILATTICE with type t := t
            );
  };
  ...
  module Bounded_Distributive_Lattice:
    BOUNDED_DISTRIBUTIVE_LATTICE with type t = toggle = {
  ...
  module Heyting_Algebra: HEYTING_ALGEBRA with type t = toggle = {
    ...
    include (
              Bounded_Distributive_Lattice:
                BOUNDED_DISTRIBUTIVE_LATTICE with type t := t
            );
  ...
  module Involutive_Heyting_Algebra:
    INVOLUTIVE_HEYTING_ALGEBRA with type t = toggle = {
  ...
  let arb_toggle: arbitrary(toggle) =
    smap(
      ...
      to_bool,
      ~newShow=
        to_bool >. Js.Json.stringifyAny >. Js.Option.getWithDefault(""),
      arb_bool,
    );
  ...
  describe("Toggle", () => {
    ...
    describe("Bounded_Distributive_Lattice", () => {
      module V =
        Verify.Bounded_Distributive_Lattice(Bounded_Distributive_Lattice);
      property3(
        "should satisfy distributivity",
        arb_toggle,
        arb_toggle,
    ...
    describe("Involutive_Heyting_Algebra", () => {
      module V =
        Verify.Involutive_Heyting_Algebra(Involutive_Heyting_Algebra);
...
let arb_result:
  (arbitrary('a), arbitrary('b)) => arbitrary(Belt.Result.t('a, 'b)) =
  (arb_ok, arb_error) =>
    smap(
      e =>
        switch (e) {
...
describe("Result", () => {
  ...
  describe("Show", () =>
    it("should show the either value", () => {
      expect(Functors.ResultF.Bool.Int.Show.show(Ok(true)))
      |> to_be("true");
      expect(Functors.ResultF.Bool.Int.Show.show(Error(123)))
      |> to_be("123");
    })
  );
  ...
  describe("Bounded", () => {
    ...
    property1(
      "should satisfy bounded",
      arb_result(arb_nat, arb_float'),
      V.bounded,
    );
  });
