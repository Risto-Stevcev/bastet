...
module type SEMIGROUP_F =
  (S: SEMIGROUP) => SEMIGROUP with type t = option(S.t);
module type MONOID_F = (S: SEMIGROUP) => MONOID with type t = option(S.t);
module type QUASIGROUP_F =
...
module type TRAVERSABLE_F =
  (A: APPLICATIVE) =>
    ...
    TRAVERSABLE with
      type t('a) = option('a) and type applicative_t('a) = A.t('a);
...
module Traversable = (A: APPLICATIVE) => {
  ...
  let traverse = (f, x) =>
    maybe(~f=A.map(a => Some(a)) <. f, ~default=A.pure(None), x)
  and sequence = x =>
    maybe(~f=A.map(a => Some(a)), ~default=A.pure(None), x);
};
