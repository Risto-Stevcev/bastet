...
module Fold_Map = (M: Interface.MONOID, F: FOLD) => {
  ...
  let fold_map_default_left = (f, x) =>
    I.(F.fold_left((acc, x) => acc <:> f(x), M.empty, x))
  and fold_map_default_right = (f, x) =>
    I.(F.fold_right((x, acc) => f(x) <:> acc, M.empty, x));
};
...
module Fold_Map_Plus = (P: Interface.PLUS, F: FOLD) => {
  ...
  let fold_map_default_left = (f, x) =>
    I.(F.fold_left((acc, x) => acc <|> f(x), P.empty, x))
  and fold_map_default_right = (f, x) =>
    I.(F.fold_right((x, acc) => f(x) <|> acc, P.empty, x));
};
