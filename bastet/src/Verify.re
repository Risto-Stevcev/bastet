...
module Compare = {
  ...
  module Semigroup_Any =
         (S: SEMIGROUP_ANY, E: EQ1 with type t('a) = S.t('a)) => {
  ...
  module Quasigroup = (Q: QUASIGROUP, E: EQ with type t = Q.t) => {
    ...
    let cancellative: (Q.t, Q.t, Q.t) => bool =
      (a, b, c) =>
        I.(!E.eq(a <:> b, a <:> c) || E.eq(b, c))
        && I.(!E.eq(b <:> a, c <:> a) || E.eq(b, c));
  };
  module Quasigroup_Any =
         (Q: QUASIGROUP_ANY, E: EQ1 with type t('a) = Q.t('a)) => {
  ...
  module Group = (G: GROUP, E: EQ with type t = G.t) => {
    ...
    let invertibility: G.t => bool =
      a =>
        I.(
          E.eq(G.inverse(a) <:> a, G.empty)
          && E.eq(a <:> G.inverse(a), G.empty)
        );
    /** via {!Interface.LOOP} */
  ...
  module Abelian_Group = (A: ABELIAN_GROUP, E: EQ with type t = A.t) => {
    let commutativity: (A.t, A.t) => bool =
      (a, b) => E.eq(A.append(a, b), A.append(b, a));
  };
  module Abelian_Group_Any =
         (A: ABELIAN_GROUP_ANY, E: EQ1 with type t('a) = A.t('a)) => {
    let commutativity: (A.t('a), A.t('a)) => bool =
      (a, b) => E.eq(A.append(a, b), A.append(b, a));
  };
  module Functor = (F: FUNCTOR, E: EQ1 with type t('a) = F.t('a)) => {
    let (<.) = Function.Infix.(<.);
    let identity: F.t('a) => bool =
      a => E.eq(F.map(Function.Category.id, a), a);
  ...
  module Apply = (A: APPLY, E: EQ1 with type t('a) = A.t('a)) => {
    ...
    let associative_composition:
      (A.t('b => 'c), A.t('a => 'b), A.t('a)) => bool =
      (f, g, h) =>
        I.(
          E.eq(
            A.map(Function.Semigroupoid.compose, f) <*> g <*> h,
            f <*> (g <*> h),
          )
        );
  };
  module Applicative = (A: APPLICATIVE, E: EQ1 with type t('a) = A.t('a)) => {
    module I = Infix.Apply(A);
    let identity: A.t('a) => bool =
      a => I.(E.eq(A.pure(Function.Category.id) <*> a, a));
  ...
  module Alt = (A: ALT, E: EQ1 with type t('a) = A.t('a)) => {
    ...
    let distributivity: ('a => 'b, A.t('a), A.t('a)) => bool =
      (f, a, b) =>
        I.(E.eq(A.map(f, a <|> b), A.map(f, a) <|> A.map(f, b)));
  };
  module Plus = (P: PLUS, E: EQ1 with type t('a) = P.t('a)) => {
    module I = Infix.Alt(P);
    let annihalation: ('a => 'b) => bool =
      f => E.eq(P.map(f, P.empty), P.empty);
  ...
  module Alternative = (A: ALTERNATIVE, E: EQ1 with type t('a) = A.t('a)) => {
    ...
    let annihalation: A.t('a => 'b) => bool =
      f => I.(E.eq(A.empty <*> f, A.empty));
  };
  module Semigroupoid =
         (S: SEMIGROUPOID, E: EQ2 with type t('a, 'b) = S.t('a, 'b)) => {
  ...
  module Quasireflexive_Eq = (E: QUASIREFLEXIVE_EQ) => {
    module I = Infix.Eq(E);
    let quasireflexivity: (E.t, E.t) => bool =
      (a, b) => I.(!(a =|= b) || a =|= a && b =|= b);
    let symmetry: (E.t, E.t) => bool = (a, b) => I.(a =|= b == (b =|= a));
    let transitivity: (E.t, E.t, E.t) => bool =
      (a, b, c) => I.(!(a =|= b && b =|= c) || a =|= c);
  ...
  module Join_Semilattice = (J: JOIN_SEMILATTICE, E: EQ with type t = J.t) => {
    ...
    let commutativity: (J.t, J.t) => bool =
      (a, b) => E.eq(J.join(a, b), J.join(b, a));
  ...
  module Lattice = (L: LATTICE, E: EQ with type t = L.t) => {
    let absorption: (L.t, L.t) => bool =
      (a, b) =>
        E.eq(L.meet(a, L.join(a, b)), a)
        && E.eq(L.join(a, L.meet(a, b)), a);
  };
  ...
  module Distributive_Lattice =
         (L: DISTRIBUTIVE_LATTICE, E: EQ with type t = L.t) => {
  ...
  module Heyting_Algebra = (H: HEYTING_ALGEBRA, E: EQ with type t = H.t) => {
    ...
    let (<|=) = O.(<|=);
    let pseudocomplement: H.t => bool =
      a => E.eq(H.(!)(a), H.implies(a, H.bottom));
    let relative_pseudocomplement: (H.t, H.t, H.t) => bool =
      (a, b, c) => H.meet(c, a) <|= b == (c <|= H.implies(a, b));
  };
  module Involutive_Heyting_Algebra =
         (H: HEYTING_ALGEBRA, E: EQ with type t = H.t) => {
    let involution: H.t => bool = a => E.eq(H.(!)(H.(!)(a)), a);
  };
  module Boolean_Algebra = (B: BOOLEAN_ALGEBRA, E: EQ with type t = B.t) => {
    let excluded_middle: B.t => bool =
      a => E.eq(B.join(a, B.(!)(a)), B.top);
  };
  ...
  module Ring = (R: RING, E: EQ with type t = R.t) => {
    ...
    let additive_inverse: R.t => bool =
      a => I.(E.eq(R.zero |-| a |+| a, R.zero));
  };
  ...
  module Division_Ring = (R: DIVISION_RING, E: EQ with type t = R.t) => {
    ...
    let multiplicative_inverse: R.t => bool =
      a => I.(E.eq(R.reciprocal(a) |*| a, R.one));
  };
  module Euclidean_Ring = (R: EUCLIDEAN_RING, E: EQ with type t = R.t) => {
    ...
    let integral_domain: (R.t, R.t) => bool =
      (a, b) =>
        I.(!(!E.eq(a, R.zero) && b != R.zero) || !E.eq(a |*| b, R.zero));
    let non_negative_degree: R.t => bool =
      a => E.eq(a, R.zero) || R.degree(a) >= 0;
    let remainder: (R.t, R.t) => bool =
      (a, b) =>
        I.(
          if (b != R.zero) {
            let q = a |/| b;
  ...
  module Contravariant =
         (C: CONTRAVARIANT, E: EQ1 with type t('a) = C.t('a)) => {
    let id = Function.Category.id;
    let (<.) = Function.Infix.(<.);
  ...
  module Profunctor =
         (P: PROFUNCTOR, E: EQ2 with type t('a, 'b) = P.t('a, 'b)) => {
    let id = Function.Category.id;
    let ((<.), (>.)) = Function.Infix.((<.), (>.));
    let identity: P.t('a, 'b) => bool = a => E.eq(P.dimap(id, id, a), a);
    let composition:
      ('a => 'b, 'c => 'd, 'e => 'a, 'd => 'f, P.t('b, 'c)) => bool =
      (f1, g1, f2, g2, a) =>
        E.eq(
  ...
  module Monad_Plus = (M: MONAD_PLUS, E: EQ1 with type t('a) = M.t('a)) => {
    let distributivity: ('a => M.t('b), M.t('a), M.t('a)) => bool =
      (f, a, b) =>
        E.eq(
          M.flat_map(M.alt(a, b), f),
          M.alt(M.flat_map(a, f), M.flat_map(b, f)),
        );
  };
  ...
  module Comonad = (C: COMONAD, E: EQ1 with type t('a) = C.t('a)) => {
    let identity: (C.t('a) => 'a, C.t('a)) => bool =
      (f, a) =>
        E.eq(C.extend(C.extract, a), a)
        && E.eq(C.extract(C.extend(f, a)), f(a));
  };
  module Bifunctor =
         (B: BIFUNCTOR, E: EQ2 with type t('a, 'b) = B.t('a, 'b)) => {
    let id = Function.Category.id;
    let (<.) = Function.Infix.(<.);
    let identity: B.t('a, 'b) => bool = a => E.eq(B.bimap(id, id, a), a);
    let composition:
      ('b => 'e, 'd => 'f, 'a => 'b, 'c => 'd, B.t('a, 'c)) => bool =
      (f1, g1, f2, g2, a) =>
        E.eq(
  ...
  module Bicontravariant =
         (B: BICONTRAVARIANT, E: EQ2 with type t('a, 'b) = B.t('a, 'b)) => {
    let id = Function.Category.id;
    let (<.) = Function.Infix.(<.);
    let identity: B.t('a, 'b) => bool = a => E.eq(B.bicmap(id, id, a), a);
    let composition:
      ('e => 'b, 'f => 'd, 'b => 'a, 'd => 'c, B.t('a, 'c)) => bool =
      (f1, g1, f2, g2, a) =>
        E.eq(
