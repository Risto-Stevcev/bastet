...
let result: ('a => 'c, 'b => 'c, Belt.Result.t('a, 'b)) => 'c =
  (f, g, a) =>
    switch (f, g, a) {
    | (f, _, Ok(a')) => f(a')
    | (_, g, Error(a')) => g(a')
...
module type MAGMA_F =
  (T: TYPE, M: MAGMA) => MAGMA with type t = Belt.Result.t(M.t, T.t);
module type MEDIAL_MAGMA_F =
  (T: TYPE, M: MAGMA) => MEDIAL_MAGMA with type t = Belt.Result.t(M.t, T.t);
module type SEMIGROUP_F =
  (T: TYPE, S: SEMIGROUP) => SEMIGROUP with type t = Belt.Result.t(S.t, T.t);
module type FUNCTOR_F =
  (T: TYPE) => FUNCTOR with type t('a) = Belt.Result.t('a, T.t);
module type APPLY_F =
  (T: TYPE) => APPLY with type t('a) = Belt.Result.t('a, T.t);
module type APPLICATIVE_F =
  (T: TYPE) => APPLICATIVE with type t('a) = Belt.Result.t('a, T.t);
module type MONAD_F =
  (T: TYPE) => MONAD with type t('a) = Belt.Result.t('a, T.t);
module type ALT_F =
  (T: TYPE) => ALT with type t('a) = Belt.Result.t('a, T.t);
module type EXTEND_F =
  (T: TYPE) => EXTEND with type t('a) = Belt.Result.t('a, T.t);
module type SHOW_F =
  (Ok: SHOW, Error: SHOW) => SHOW with type t = Belt.Result.t(Ok.t, Error.t);
module type EQ_F =
  (Ok: EQ, Error: EQ) => EQ with type t = Belt.Result.t(Ok.t, Error.t);
module type ORD_F =
  (Ok: ORD, Error: ORD) => ORD with type t = Belt.Result.t(Ok.t, Error.t);
module type BOUNDED_F =
  (Ok: BOUNDED, Error: BOUNDED) =>
  BOUNDED with type t = Belt.Result.t(Ok.t, Error.t);
module type FOLDABLE_F =
  (T: TYPE) => FOLDABLE with type t('a) = Belt.Result.t('a, T.t);
module type TRAVERSABLE_F =
  (T: TYPE, A: APPLICATIVE) =>
  TRAVERSABLE with type t('a) = Belt.Result.t('a, T.t);
module type BITRAVERSABLE_F =
  (A: APPLICATIVE) =>
  BITRAVERSABLE with
    type t('a, 'b) = Belt.Result.t('a, 'b) and
...
module Many_Valued_Logic = {
  ...
  module type EQ_F =
    (Ok: TYPE, Error: TYPE) => EQ with type t = Belt.Result.t(Ok.t, Error.t);
  module type ORD_F =
    (Ok: TYPE, Error: TYPE) => ORD with type t = Belt.Result.t(Ok.t, Error.t);
  ...
  module Bounded_Lattice = (Ok: BOUNDED_LATTICE, Error: BOUNDED_LATTICE) => {
    ...
    include (
              Bounded_Meet_Semilattice(Ok, Error):
                (module type of Bounded_Meet_Semilattice(Ok, Error)) with
                  type t := t
            );
  };
  ...
  module Bounded_Distributive_Lattice =
         (Ok: BOUNDED_LATTICE, Error: BOUNDED_LATTICE) => {
    ...
    (Ok: HEYTING_ALGEBRA, Error: HEYTING_ALGEBRA) => {
      ...
      include (
                Bounded_Distributive_Lattice(Ok, Error):
                  (module type of Bounded_Distributive_Lattice(Ok, Error)) with
                    type t := t
              );
...
module Unsafe = {
  let from_ok = a =>
    ...
    | Ok(a') => a'
    | _ =>
      Js.Exn.raiseTypeError("You passed in an `Error` value to `from_ok`")
    }
  and from_error = a =>
    switch (a) {
    | Error(a') => a'
    | _ =>
      Js.Exn.raiseTypeError("You passed in an `Ok` value to `from_error`")
    };
};
