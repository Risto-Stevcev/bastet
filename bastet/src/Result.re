...
let result: ('a => 'c, 'b => 'c, result('a, 'b)) => 'c =
  (f, g, a) =>
    switch (f, g, a) {
    | (f, _, Ok(a')) => f(a')
    | (_, g, Error(a')) => g(a')
...
module type MAGMA_F =
  (T: TYPE, M: MAGMA) => MAGMA with type t = result(M.t, T.t);
module type MEDIAL_MAGMA_F =
  (T: TYPE, M: MAGMA) => MEDIAL_MAGMA with type t = result(M.t, T.t);
module type SEMIGROUP_F =
  (T: TYPE, S: SEMIGROUP) => SEMIGROUP with type t = result(S.t, T.t);
module type FUNCTOR_F =
  (T: TYPE) => FUNCTOR with type t('a) = result('a, T.t);
module type APPLY_F =
  (T: TYPE) => APPLY with type t('a) = result('a, T.t);
module type APPLICATIVE_F =
  (T: TYPE) => APPLICATIVE with type t('a) = result('a, T.t);
module type MONAD_F =
  (T: TYPE) => MONAD with type t('a) = result('a, T.t);
module type ALT_F =
  (T: TYPE) => ALT with type t('a) = result('a, T.t);
module type EXTEND_F =
  (T: TYPE) => EXTEND with type t('a) = result('a, T.t);
module type SHOW_F =
  (Ok: SHOW, Error: SHOW) => SHOW with type t = result(Ok.t, Error.t);
module type EQ_F =
  (Ok: EQ, Error: EQ) => EQ with type t = result(Ok.t, Error.t);
module type ORD_F =
  (Ok: ORD, Error: ORD) => ORD with type t = result(Ok.t, Error.t);
...
module Many_Valued_Logic = {
  ...
  module type EQ_F =
    (Ok: TYPE, Error: TYPE) => EQ with type t = result(Ok.t, Error.t);
  module type ORD_F =
    (Ok: TYPE, Error: TYPE) => ORD with type t = result(Ok.t, Error.t);
...
module Choose = (A: ALT) => {
  let choose: (A.t('a), A.t('b)) => A.t(result('a, 'b)) =
...
module Unsafe = {
  let from_ok = a =>
    ...
    | Ok(a') => a'
    | _ =>
      raise(Invalid_argument("You passed in an `Error` value to `from_ok`"))
    }
  and from_error = a =>
    switch (a) {
    | Error(a') => a'
    | _ =>
      raise(Invalid_argument("You passed in an `Ok` value to `from_error`"))
    };
};

let is_ok = a => result(const(true), const(false), a)
and is_error = a => result(const(false), const(true), a)
and note: ('err, option('a)) => result('a, 'err) =
  default => Option.maybe(~f=x => Ok(x), ~default=Error(default))
and hush: result('a, 'err) => option('a) =
