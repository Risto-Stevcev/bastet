...
module ComparePromise = {
  ...
  let eq = (a, b) =>
    Js.Promise.then_(
      a' => Js.Promise.then_(b' => Js.Promise.resolve(a' == b'), b),
      a,
    )
    |> Obj.magic;
};
...
describe("Promise", () => {
  ...
  describe("Applicative", () => {
    module V =
      Verify.Compare.Applicative(Promise.Applicative, ComparePromise);
    async_property1(
      "should satisfy identity",
      arb_nat,
      Obj.magic <. V.identity <. promise,
  ...
  describe("Monad (unlawful)", () => {
    ...
    async_property1(
      "will *seemingly* properly flatten (not correct)", arb_nat, n =>
      Promise.Infix.(
        Fn.flatten(pure(pure(n)))
        >>= (flattened_n => pure(flattened_n == n))
      )
    );
    async_property1(
      "promises are not actually nested despite the type signature", arb_nat, n =>
      Promise.Infix.(
        pure(pure(n))
