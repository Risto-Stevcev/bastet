...
module Functor = (F: FUNCTOR) => {
  ...
  and void_right: ('a, F.t('b)) => F.t('a) =
    (a, fb) => F.map(const(a), fb)
  and void_left: (F.t('a), 'b) => F.t('b) = (fa, b) => F.map(const(b), fa)
  and flap: (F.t('a => 'b), 'a) => F.t('b) =
    (fs, a) => F.map(f => f(a), fs);
};
...
module Apply = (A: APPLY) => {
  ...
  let apply_first: (A.t('a), A.t('b)) => A.t('a) =
    (a, b) => const <$> a <*> b
...
module Monad = (M: MONAD) => {
  ...
  and compose_kliesli: ('a => M.t('b), 'b => M.t('c), 'a) => M.t('c) =
    (f, g, a) => I.(f(a) >>= g)
  and compose_kliesli_flipped: ('b => M.t('c), 'a => M.t('b), 'a) => M.t('c) =
    (f, g, a) => I.(f =<< g(a))
  and if_m: (M.t(bool), M.t('a), M.t('a)) => M.t('a) =
    (p, t, f) => I.(p >>= (p' => p' ? t : f))
  and liftM1: ('a => 'b, M.t('a)) => M.t('b) =
    (f, fa) => I.(fa >>= (fa' => M.pure(f(fa'))))
  and ap: (M.t('a => 'b), M.t('a)) => M.t('b) =
    (f, fa) => I.(f >>= (f' => fa >>= (fa' => M.pure(f'(fa')))))
  and when_: (M.t(bool), M.t(unit)) => M.t(unit) =
    (p, fa) => I.(p >>= (p' => A.when_(p', fa)))
  and unless: (M.t(bool), M.t(unit)) => M.t(unit) =
...
module Foldable = (F: FOLDABLE) => {
    ...
    and intercalate: (~separator: M.t, F.t(M.t)) => M.t =
      (~separator, xs) => {
        let go = (acc, x) =>
          ...
          | {init: _, acc: acc'} =>
            I.{init: false, acc: acc' <:> separator <:> x}
          };
  ...
  module Plus = (P: PLUS) => {
    let one_of: F.t(P.t('a)) => P.t('a) =
      fa => F.fold_right(P.alt, P.empty, fa);
  };
  ...
  module Monad = (M: MONAD) => {
    ...
    let fold_monad: (('a, 'b) => M.t('a), 'a, F.t('b)) => M.t('a) =
      (f, a, fa) =>
        I.(F.fold_left((acc, x) => acc >>= flip(f, x), M.pure(a), fa));
  };
};
