...
module type MAGMA = {type t; let append: (t, t) => t;};

module type MEDIAL_MAGMA = {include MAGMA;};

module type MAGMA_ANY = {type t('a); let append: (t('a), t('a)) => t('a);};

module type SEMIGROUP = {include MAGMA;};

module type SEMIGROUP_ANY = {include MAGMA_ANY;};
...
module type MONOID_ANY = {include SEMIGROUP_ANY; let empty: t('a);};

module type QUASIGROUP = {include MAGMA;};
...
module type QUASIGROUP_ANY = {include MAGMA_ANY;};
...
module type GROUP = {
  ...
  include LOOP;
  /* or */ include MONOID with type t := t;
  let inverse: t => t;
};
...
module type GROUP_ANY = {
  /* This is for documentation purposes only, include either LOOP or MONOID */
  include LOOP_ANY;
  /* or */ include MONOID_ANY with type t('a) := t('a);
  let inverse: t('a) => t('a);
};
...
module type ABELIAN_GROUP = {include GROUP;};
module type ABELIAN_GROUP_ANY = {include GROUP_ANY;};
...
module type APPLICATIVE = {include APPLY; let pure: 'a => t('a);};
...
module type ALT = {include FUNCTOR; let alt: (t('a), t('a)) => t('a);};

module type PLUS = {include ALT; let empty: t('a);};
...
module type TRAVERSABLE_F =
  (A: APPLICATIVE) => TRAVERSABLE with type applicative_t('a) = A.t('a);
...
module type CATEGORY = {include SEMIGROUPOID; let id: t('a, 'a);};

module type EQ = {type t; let eq: (t, t) => bool;};

module type QUASIREFLEXIVE_EQ = {include EQ;};

module type EQ1 = {type t('a); let eq: (t('a), t('a)) => bool;};

module type EQ2 = {type t('a, 'b); let eq: (t('a, 'b), t('a, 'b)) => bool;};
...
let int_to_ordering = x =>
  x < 0 ? `less_than : x == 0 ? `equal_to : `greater_than;

let unsafe_compare = (a, b) =>
  a < b ? `less_than : a == b ? `equal_to : `greater_than;
...
module Ordering = (O: ORD) => {
  ...
  and greater_than: (O.t, O.t) => bool =
    (a, b) => O.compare(a, b) == `greater_than
  and less_than_or_equal: (O.t, O.t) => bool =
    (a, b) => O.compare(a, b) != `greater_than
  and greater_than_or_equal: (O.t, O.t) => bool =
    (a, b) => O.compare(a, b) != `less_than;
};

module type BOUNDED = {include ORD; let top: t; let bottom: t;};

module type JOIN_SEMILATTICE = {type t; let join: (t, t) => t;};

module type MEET_SEMILATTICE = {type t; let meet: (t, t) => t;};
...
module type BOUNDED_MEET_SEMILATTICE = {include MEET_SEMILATTICE; let top: t;};
...
module type DISTRIBUTIVE_LATTICE = {include LATTICE;};

module type BOUNDED_DISTRIBUTIVE_LATTICE = {include BOUNDED_LATTICE;};
...
module type INVOLUTIVE_HEYTING_ALGEBRA = {include HEYTING_ALGEBRA;};

module type BOOLEAN_ALGEBRA = {include HEYTING_ALGEBRA;};
...
module type RING = {include SEMIRING; let subtract: (t, t) => t;};

module type COMMUTATIVE_RING = {include RING;};

module type DIVISION_RING = {include RING; let reciprocal: t => t;};
...
module type MONAD_PLUS = {include MONAD_ZERO;};
...
module type COMONAD = {include EXTEND; let extract: t('a) => 'a;};
...
module type BIFOLDABLE = {
  ...
  module Fold_Map:
    (M: MONOID) => {let fold_map: ('a => M.t, 'b => M.t, t('a, 'b)) => M.t;};
  module Fold_Map_Any:
    (M: MONOID_ANY) =>
...
module type BITRAVERSABLE = {
  ...
  let bitraverse:
    ('a => applicative_t('c), 'b => applicative_t('d), t('a, 'b)) =>
    applicative_t(t('c, 'd));
  let bisequence:
    t(applicative_t('a), applicative_t('b)) => applicative_t(t('a, 'b));
};

module type BITRAVERSABLE_F =
  (A: APPLICATIVE) => BITRAVERSABLE with type applicative_t('a) = A.t('a);
